...
- `ConversationCoordinator.start(conversationId, npcId)` validates the shared catalog and starts only from `playing`; duplicate requests and empty placeholder definitions are rejected before lifecycle events or state/camera ownership. If a start observer fails after another presentation owner initialized, the coordinator rolls the partial session back through the cancellation path.
...
The dialogue panel has a viewport-bounded stable height. Speaker changes and typewriter growth do not move it vertically; unusually long text scrolls inside the text region while portraits and reveal/continue/cancel controls remain visible.
