...
The player/vehicle perimeter is divided into 12 contiguous visual/collider pairs on the exact new edges. Each pair is uniquely owned by a nearby protected WORLD-004 sector, and every physical corner of every pair falls within the adaptive policy's current 28m hard-near radius. The unsplit cross-axis west road is uniquely reassigned to nearby `sector.west-rim-north`, keeping all four of its footprint corners within that radius while preserving the ten-road map contract. The north pedestrian exit is re-authored against the new bounds on a visibly supported sidewalk that extends 0.7m beyond the edge, enough for 0.4m authored clearance plus the 0.3m pedestrian body radius. Adaptive selection, adjacency, mission protection, hysteresis, retry, LOD, texture cloning, and disposal stay in `LevelSystem` and `AdaptiveSectorStreamingPolicy`.
...
Both floors rise 0.2m above the surrounding slab, below the shared 0.28m step limit. Roof colliders carry the existing `roof` semantic so collision treats them as head/camera cover rather than ground. The only public visual extension is optional `BoxVisualDefinition.materialName`, allowing code-native fixture boxes to participate in the existing time-of-day emissive binder. The two point lights bring the existing level maximum to four and remain shadow-free. `TimeOfDayLightingSystem` remains the sole light authority: its four PointLights are level-owned, while sector-loaded events bind matching emissive materials after scene attachment and sector-unloaded events restore and release those references before sector resource disposal. Fixture/material owner sets make duplicate events idempotent and preserve shared materials until their final fixture unloads.
...
`pnpm validate:world-004` checks plan/runtime/map agreement, exact dimensions, counts, local texture metadata, unique ownership, protected-radius boundary/road extents, level validity, and four traffic endpoints. Unit tests cover hard-pressure sector selection plus live scene/collision residency at outer corners, pre-disposal unload ordering, streamed fixture bind/release/reload cycles, shared-material reference counting, bounds, outer walls, spawns, camera clearance, interiors, occupants, lighting, maps, traffic, and previous milestones.
...
- The two interior PointLights remain level-owned under the existing four-light ceiling; only their sector-owned emissive material bindings stream. A future per-sector light authority should replace this contract only if profiling justifies the additional lifecycle complexity.
