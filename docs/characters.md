...
`CharacterPlayerVisual` feeds movement and one-shot intent into the game-owned `CharacterAnimationStateMachine`. Its fixed priority is reaction, action, airborne/landing, then locomotion; `CharacterLocomotionPolicy` separately decides whether the selected action owns the full body or the reviewed firearm upper-body layer. It records every transition, fallback, and restoration while the visual remains responsible only for mixer playback. Steering never selects or restarts a directional clip. `PlayerMovementSimulation` smooths its authoritative simulation/action heading toward the actual accelerated horizontal velocity and applies walk/run state hysteresis; the visual simply mirrors that heading. Missing requests fall back to idle, then to a valid static pose. `CharacterLoader` reports every missing authored mapping. After each mixer update the definition's root offset is restored, so authored root-motion tracks cannot translate the visual away from the simulation transform. See [firearm locomotion and action layering](./shooting-locomotion.md).
