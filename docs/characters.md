...
`CharacterPlayerVisual` feeds movement and one-shot intent into the game-owned `CharacterAnimationStateMachine`. Its fixed priority is reaction, action, airborne/landing, then locomotion; it records every transition, fallback, and restoration while the visual remains responsible only for mixer playback. Running always resolves the normal forward `run` clip, so steering does not restart mixer playback. `PlayerMovementSimulation` smooths its authoritative simulation/action heading toward the actual accelerated horizontal velocity; the visual simply mirrors that heading. Missing requests fall back to idle, then to a valid static pose. `CharacterLoader` reports every missing authored mapping. After each mixer update the definition's root offset is restored, so authored root-motion tracks cannot translate the visual away from the simulation transform.
